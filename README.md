...
| 合成复用原则（CAPP）<br />）                       | 尽量使用组合实现代码复用，而不使用继承             | 降低代码耦合                               |
...
| 设计模式                | 一句话归纳                                 | 目的 | 生活案例 | 框架源码举例                                        |
| ----------------------- | ------------------------------------------ | ---- | -------- | --------------------------------------------------- |
| 代理模式（Proxy）       | 媒婆                                       |      |          | ProxyFactoryBean、JdkDynamicAopProxy、CglibAopProxy |
...
抽象工厂形式上可能会出现抽象方法，但是并不能体现流程

模版方法：也是可能出现抽象方法，但是一定某个流程中一个步骤
...
适配器：解决兼容问题，可以继承也可以组合

中介者：一定是用组合，所有人都可能持有中介者对象
...
组合：目的不是建立连接，而是为了为一个整体统一行为，统一一套API

#### 门面模式VS装饰器模式

门面：统一入口，打开一扇窗，通向全世界。背后有很多子系统，有点代理的意思
